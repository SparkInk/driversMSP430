...
char *usciA1UartGets (char * rxString) {
   ...
   do {   // writing a string in the console
        ...
        /* 
            if I ever get to this part of the code
            please, me, replace the spiRxBuff, with a static 
            inside of this function. Not a good programming habbit
         */

        spiRxBuff[i] = UCA1RXBUF;    // move an entered character to rxBuffer
   ...
   spiRxBuff[i] = '\n'; // puts a new-line character at the end
    ...
    strcpy(rxString, spiRxBuff); // copy rxBuffer to rxString
    memset(spiRxBuff, 0, BUF_SIZE);  // clear rxBuffer for the next command
