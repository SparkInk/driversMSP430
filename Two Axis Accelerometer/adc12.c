...
void rightShift(unsigned int *array){

	volatile unsigned char i;

	for (i = 1; i < ARRAY_SIZE; i++) {
		array[ARRAY_SIZE - i] = array[ARRAY_SIZE - (i + 1)];
	}
...
__interrupt void eocADC12ISR(void)	{
	...
	// volatile static unsigned char i = 1;
	// volatile static unsigned char j = 1;

	// needs to be modified for channel selection with a proper interrupt handler.
	switch(_even_in_range(ADC12IV, 10)) {
	...
	case 6:    // ADC12MEM0; Interrupt Flag: ADC12IFG0; P6.0
		...
		// // store X-axis values
	    // mem0Result[1] = ADC12MEM0;		

		// // the first value of the array is used 
		// // to calculate the real angle
		mem0Result[0] = ADC12MEM0;
		rightShift(mem0Result);

		update = 1;
	    break;
	case 10:    // ADC12MEM2; Interrupt Flag: ADC12IFG1; P6.2

		// // store Y-axis valuess
        // mem1Result[j] = ADC12MEM2;
		// j++;

		// // the first value of the array is used 
		// // to calculate the real angle
		mem1Result[0] = ADC12MEM2;
		rightShift(mem1Result);
