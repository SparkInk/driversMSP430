...
#define VERT_HORZ_ADDR          BIT1


#define LCD_ROW_IN_BANK 8 	    // 8 rows in a bank. 6 banks, so  8x6 = 48 rows of pixels. y coordinate
#define _NOK_LCD_RST        P6OUT &= ~BIT1; P6OUT |= BIT1;  // reset strobe
#define _NOK_LCD_PWR_OFF    P6OUT &= ~BIT2; // turn the LCD OFF
#define _NOK_LCD_PWR_ON     P6OUT |= BIT2;  // turn the LCD ON
#define nSCE     P4OUT   // SCE is ACTIVE LOW
#define DnC     P4OUT   // D (data) is active HIGH; nC is active LOW
/************************************************************************************
* Function: nokLcdInit
* -
* argument:
...
* Modified: <date of any mods> usually taken care of by rev control
************************************************************************************/
void nokLcdWrite(int lcdByte, char cmdType);
...
unsigned int nokLcdDrawScrnLine (int coorValue, int lineName);
